...
if news_type == 'None':
    ...
    if yt_type == 'Video':
        ...
        try:
            # time_option = st.radio('Search Time Limit', ['6 hours ago', '1 day ago', '7 days ago', '2 weeks ago', '1 month ago', '4 months ago'])
            article_limit = st.number_input('Article Limit', 0, 1000)
            channel_id = st.text_input('Enter Youtube Channel id. e.g. UC16niRr50-MSBwiO3YDb3RA for BBC')
            if channel_id:
                channel_dict = scrapetube.get_channel(channel_id, limit=article_limit)
